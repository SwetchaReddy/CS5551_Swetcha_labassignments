...
app.get('/getVenues', function (req, res) {
    ...
    request('https://api.foursquare.com/v2/venues/search?client_id=Q0ENF1YHFTNPJ31DCF13ALLENJW0P5MTH13T1SA0ZP1MUOCI&client_secret=ZH4CRZNEWBNTALAE3INIB5XG0QI12R4DT5HKAJLWKYE1LHOG&v=20160215&limit=3&near=Kansas&query=halloween', function (error, response, body) {
        //Check for error
        if(error){
...
app.post('/register', function (req, res) {
    ...
    app.post('/update', function (req, res) {
        ...
        var updateDocument = function (db, data, callback) {
            db.collection('user').updateOne({"data.response.venues[i].name": ven[i].name}, {$set: item}, function (err, result) {
                if (err) {
                    res.write(" Failed");
                    res.end();
                }
    ...
    app.post('/delete', function (req, res) {
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            db.collection('user').deleteOne({"data.response.venues[i].name": ven[i].name}, function (err, result) {
